...
from opulence.facts import Phone, Country
...
class PhoneInfoga(ScriptCollector):
    ...
    def launch(self, fact):
        cmd_ovh = [self._script_path_, "--no-ansi", "--scanner", "ovh", "--number", fact.number.value]
        cmd_numverify = [self._script_path_, "--no-ansi", "--scanner", "numverify", "--number", fact.number.value]
    ...
    def parse_local_result(self, result):
        res = re.findall("Running local scan...\\n\\[\\+\\] International format: (.*)\\n\\[\\+\\] Local format: (.*)\\n\\[\\+\\] Country found: (.*) \\((.*)\\)\\n\\[\\+\\] City\\/Area: (.*)\\n\\[\\+\\] Carrier: (.*)\\n\\[\\+\\] Timezone: (.*)\\n", result)
        if res:
            for info in res:
                il_format, local_format, country, country_code, area, carrier, timezone = info
                yield Country(name=country, timezone=timezone, area=area)
                yield Phone(number=il_format, localformat=local_format, carrier=carrier, country_code=country_code)
    def parse_numverify_result(self, result, number):
        res = re.findall("Running Numverify.com scan...\\n\\[\\+\\] Number: \\((.*)\\) (.*)\\n\\[\\+\\] Country: (.*) \\((.*)\\)\n\\[\\+\\] Location: (.*)\\n\\[\\+\\] Carrier: (.*)\\n\\[\\+\\] Line type: (.*)\\n", result)
        if res:
            for info in res:
